...
async function renderPhotosList() {
    eraseContent();
    $("#newPhotoCmd").show();
    $("#content").append("<button class='modifyButton' data-photo-id='eaefb490-99f1-11ee-90d3-114082fc2e65'>Modifier</button>" +
                         "<button class='deleteButton' data-photo-id='eaefb490-99f1-11ee-90d3-114082fc2e65'>Supprimer</button>");
...
function renderModifyPhoto(photoId) {
    ...
    API.GetPhotosById(photoId).then(
        (photo) => {
            eraseContent();
            UpdateHeader("Modification Photo", "updatePhoto");
            $("#newPhotoCmd").hide();
            $("#content").append(`
            <br>
            <form class="form" id="modifyPhotoForm">
                <fieldset>
                    ...
                    <label for="Share" style="user-select: none"  >Photo Partagé</label>
                </fieldset>
                <fieldset>
                    <legend>Image</legend>
...
function renderDeletePhoto(photoId){
    ...
    API.GetPhotosById(photoId).then(
        (photo) => {
            eraseContent();
            UpdateHeader("Supprimer Photo", "deletePhoto");
            $("#newPhotoCmd").hide();
            $("#content").append(`
            <div class="content loginForm">
                <br>
                <div class="form">
                 <h2>La photo sera supprimer. Êtes vous certain de vouloir le faire? </h2>
                 <div class="photoTitle">${photo.Title}</div>
                 <img class="photoDetailsLargeImage" src="${photo.Image}" alt="${photo.Description}">
                 <button class="form-control btn-danger" id="deletePhotoCmd">Supprimer la photo</button>
                 <br>
...
async function renderCreatePhoto() {
    timeout();
    eraseContent();
    UpdateHeader("Ajouter Photo", "createPhoto");
    $("#newPhotoCmd").hide();
    $("#content").append(`
    <br>
